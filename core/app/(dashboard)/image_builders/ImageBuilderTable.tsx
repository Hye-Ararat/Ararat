...
export default function ImageBuilderTable({imageBuilders} : {imageBuilders: Prisma.ImageBuilderGetPayload<{}>[]}) {
    ...
    imageBuilders.forEach((imageBuilder, index) => {
        rows.push([
            <Typography key={index} sx={{m: "auto"}} fontWeight="bold">{imageBuilder.name}</Typography>,
            <Typography key={index} sx={{m: "auto"}}>{imageBuilder.url}</Typography>,
            <>
