...
export default class ImageServer {
  ...
  get id() {
    return new Promise(async (resolve, reject) => {
      if (this.imageServer.includes("http")) {
        ...
        return resolve(this.imageServer);
      }
    });
  }
  ...
  get useImages() {
    return new Promise(async (resolve, reject) => {
      let globalPerms = await this.user.permissions;
      if (globalPerms.includes("use-images_image-server")) return resolve(true);
      return resolve((await this.permissions).includes("use-images_image-server"));
    });
  }
}
