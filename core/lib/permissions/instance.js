...
import InstanceUser from "./instance/user";
...
export default class Instance {
    ...
    get start() {
        return new Promise(async (resolve, reject) => {
            if ((await this.user.permissions).includes("start_instance")) {
                return resolve(true);
            }
            if ((await this.permissions).includes("start_instance")) {
                return resolve(true)
            }
            return resolve(false);
        })
    ...
    get stop() {
        return new Promise(async (resolve, reject) => {
            if ((await this.user.permissions).includes("stop_instance")) {
                return resolve(true);
            }
            if ((await this.permissions).includes("stop_instance")) {
                return resolve(true)
            }
            return resolve(false);
        })
    }
    instanceUser(instanceUserId) {
        return new InstanceUser(this, this.user, instanceUserId);
    }
}