...
export default class Volume {
    ...
    get attach() {
        return new Promise(async (resolve, reject) => {
            ...
            if (permissions.includes("attach_volume")) {
                return resolve(true);
            }
            const nodePermissions = await this.node.permissions;
            if (nodePermissions.includes("attach_volume")) {
                return resolve(true);
            }
            let storagePool = await this.storagePool.data;
            if (storagePool) {
                if (storagePool.metadata.config["user.permissions"]) {
                    if (JSON.parse(storagePool.metadata.config["user.permissions"])[this.user.user].includes("attach_volume")) {
                        return resolve(true);
                    }
                }
            }
            ...
            if (volume) {
                if (volume.config["user.permissions"]) {
                    if (JSON.parse(volume.config["user.permissions"])[this.user.user].includes("attach_volume")) {
                        return resolve(true);
                    }
                }
            }
