...
export default function Instance({ data }) {
  ...
  var { data: instanceData } = useSWR(
    `/api/v1/client/instances/${id}?include=["magma_cube", "node", "network"]`,
    fetcher
  );
    ...
    if (instance.data) {
      ...
      if (instance.sockets.monitor) {
        ...
        instance.sockets.setMonitor(
          new WebSocket(
            `${instance.data.relationships.node.address.ssl ? "wss" : "ws"}://${
              instance.data.relationships.node.address.hostname
  ...
  return (
    <>
      ...
      <Grid container xs={12} sx={{ mt: 2 }}>
        <Grid item xs={12} sx={{ height: "100%", minHeight: "400px" }} container>
