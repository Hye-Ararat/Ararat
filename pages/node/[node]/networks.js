...
function OpenFloatingIPPort({ open, node, selectedNetwork, selectedForward, setOpen }) {
    ...
    return (
        <MultiFieldDialog
            ...
            fields={[
                {
                    ...
                    input: (
                        <Autocomplete
                            ...
                            options={selectedNetwork.leases}
                            getOptionLabel={(option) =>
                                option.instance ? option.instance.name + ` (${option.address})` : option.address
                            }
                            renderInput={(params) => (
                                <TextField variant="standard" {...params} placeholder="IP Address" sx={{ width: "244px" }} />
                            )}
                        />
                    )
                },
            ...
            action={
                description && address && protocol && targetPort && listenPort ? (
                    <Button
                        variant="contained"
                        color="success"
                        onClick={() => {
                            axios
                                .patch(
                                    `/api/v1/nodes/${node.id}/networks/${selectedNetwork.name}/forwards/${selectedForward.listen_address}`,
                                    {
                                        ports: [
...
function AddNetworkUser({ open, setOpen, node, selectedNetwork }) {
    const [user, setUser] = useState(null);
    const router = useRouter();
    useEffect(() => {
        if (user) {
            axios.get(`/api/v1/nodes/${node.id}/networks/${selectedNetwork.name}`).then((res) => {
                let data = res.data;
                console.log(data)
                let users;
    ...
    return <AddUserDialog open={open} permSection="network" userState={setUser} />;
}
export default function Networks({ node, networks }) {
    ...
    const [addingNetworkUser, setAddingNetworkUser] = useState(null);
    return (
        <>
            <Grid container direction="row" sx={{ mb: 2 }}>
                ...
                <Grid container xs={4} sm={4} md={3} sx={{ ml: "auto", mt: "auto", mb: "auto" }}>
                    <Button color="success" variant="contained" sx={{ ml: "auto" }} onClick={() => setCreatingNetwork(true)}>
                        Create Network
                    </Button>
                </Grid>
            </Grid>
            ...
            {selectedNetwork ? (
                <>
                    <OpenFloatingIPPort
                        setOpen={setOpeningFloatingIPPort}
                        open={openingFloatingIPPort}
                        node={node}
                        selectedNetwork={selectedNetwork}
                        selectedForward={selectedForward}
                    />
                    <AddNetworkUser setOpen={setAddingNetworkUser} open={addingNetworkUser} selectedNetwork={selectedNetwork} node={node} />
                </>
            ) : (
                ""
            )}
            <SideLayout
                listItems={reformatItemList(networks, "name", "name", "type", "used_by")}
                thirdItemFormatter={(item) => {
                    return (
                        networks.find((net) => net.name === item).config["ipv4.address"] +
                        " " +
                        networks.find((net) => net.name === item).config["ipv6.address"]
                    );
                }}
                FarAction={(id) => {
                    return (
                        ...
                        <>
                            <Grid container direction="column" xs={6} sx={{ mt: "auto", mb: "auto", ml: "auto" }}>
                                <Typography variant="h6" align="center">
                                    {prettyBytes(networks.find((net) => net.name == id).state.counters.bytes_sent)}
                                </Typography>
                                <Typography align="center">Sent</Typography>
                            </Grid>
                            <Grid container direction="column" xs={6} sx={{ mt: "auto", mb: "auto", ml: "auto" }}>
                                <Typography variant="h6" align="center">
                                    {prettyBytes(networks.find((net) => net.name == id).state.counters.bytes_received)}
                                </Typography>
                ...
                sections={[
                    {
                        ...
                        action: (item) => {
                            ...
                            return (
                                <Button
                                    onClick={() => setCreatingNetworkForward(true)}
                        ...
                        formatter: (item) => {
                            ...
                            return (
                                <>
                                    {networks.find((net) => net.name === item).forwards.length > 0 ? (
                                        networks
                                            .find((net) => net.name === item)
                                                    ...
                                                    <Accordion key={index} sx={{ backgroundColor: "background.default" }}>
                                                        <AccordionSummary onClick={(e) => setSelectedForward(forward)} expandIcon={<ExpandMore />}>
                                                            <Grid container direction="column">
                                                                <Typography sx={{ mt: "auto", mb: "auto" }} variant="h6" fontFamily="Poppins">
                                                                    {forward.listen_address}
                                                                </Typography>
                                                                <Typography sx={{ mt: "auto", mb: "auto" }} color="text.secondary" fontFamily="Poppins">
                                                                    {forward.description}
                                                                </Typography>
                                                            </Grid>
                                                        </AccordionSummary>
                                                        <AccordionDetails>
                                                            <Grid container direction="column">
                                                                <Grid container direction="row">
                                                                    <Typography fontWeight={600} sx={{ mb: "auto", mt: "auto" }}>
                                                                        Ports
                                                                    </Typography>
                                                                    <Button
                                                                        sx={{ ml: "auto", mb: "auto", mt: "auto" }}
                                                                        variant="contained"
                                                                        color="info"
                                                                        onClick={() => setOpeningFloatingIPPort(true)}
                                                                    >
                                                                        Open Port
                                                                    </Button>
                                                                </Grid>
                                                                        ...
                                                                        <Fade key={index} in={true}>
                                                                            <Paper sx={{ p: 1, mt: 1 }}>
                                                                                <Grid container sx={{ mt: "auto", mb: "auto" }}>
                                                                                    ...
                                                                                    <Typography sx={{ ml: "auto", mt: "auto", mb: "auto", mr: 5 }}>
                                                                                        {port.target_address}:{port.target_port} ➡️ {forward.listen_address}:
                                                                                        {port.listen_port}
                                                                                    </Typography>
                                                                                    <Button
                                                                                        variant="contained"
                                                                                        ...
                                                                                        onClick={() => {
                                                                                            axios
                                                                                                .patch(
                                                                                                    `/api/v1/nodes/${node.id}/networks/${selectedNetwork.name}/forwards/${forward.listen_address}`,
                                                                                                    {
                                                                                                        ports: [...forward.ports.filter((p, i) => i !== index)]
                                                                                                    }
                                                                                                )
                    ...
                    },
                    {
                        ...
                        action: (item) => {
                            ...
                            return (
                                <Button
                                    onClick={() => setAddingNetworkUser(true)}
                                    sx={{ ml: "auto", mt: "auto", mb: "auto" }}
                                    variant="contained"
                                    color="info"
                                >
                        ...
                        formatter: (item) => {
                            return (
                                networks.find((net) => net.name == item).config["user.permissions"] ? (
                                    Object.keys(JSON.parse(networks.find((net) => net.name == item).config["user.permissions"])).map((user, index) => {
                                        return (
                    ...
                    },
                    {
                        ...
                        formatter: (item) => {
                            return networks.find((net) => net.name === item).leases.length == 0 ? (
                                <Grid container direction="column">
                                    <Typography align="center">No Devices Connected</Typography>
                                </Grid>
                            ) : (
                                networks
                                    .find((net) => net.name === item)
                                                    ...
                                                    <Grid container>
                                                        <Grid container direction="row">
                                                            <Typography fontWeight="bold" sx={{ mr: 0.5 }}>
                                                                {lease.instance ? lease.instance.name : lease.hostname}
                                                            </Typography>
                                                                        ...
                                                                        {lease.instance.state.network ? Object.keys(lease.instance.state.network).map((key, index) => {
                                                                            return lease.instance.state.network[key].addresses.map((address, index) => {
                                                                                return lease.instance.state.network[key].addresses[index].address ==
                                                                                    lease.address ||
                                                                                    lease.instance.state.network[key].addresses[index].address ==
                                                                                    lease.address2 ? (
                                                                                    <>
                                                                    ...
                                                                    <Grid container direction="column" xs={4} sx={{ ml: "auto" }}>
                                                                        ...
                                                                        <Grid container direction="row">
                                                                            <Grid container direction="column" xs={6}>
                                                                                <Typography variant="h6" align="center">
                                                                                    {prettyBytes(lease.instance.state.total_out)}
                                                                                </Typography>
                                                                                <Typography align="center">Sent</Typography>
                                                                            </Grid>
                                                                            <Grid container direction="column" xs={6}>
                                                                                <Typography variant="h6" align="center">
                                                                                    {prettyBytes(lease.instance.state.total_in)}
                                                                                </Typography>
                                                                                <Typography align="center">Recieved</Typography>
                                                                            </Grid>
                                                                        </Grid>
