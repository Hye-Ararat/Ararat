        ...
        return new Promise(async (resolve, reject) => {
            ...
            if (storagePool) {
                if (storagePool.metadata.config["user.permissions"]) {
                    if (getLXDUserPermissions(this.user.user, JSON.parse(storagePool.metadata.config["user.permissions"])[this.user.user].includes("attach-volume"))) {
                        return resolve(true);
                    }
                }
            }
            ...
            if (volume) {
                if (volume.config["user.permissions"]) {
                    if (getLXDUserPermissions(this.user.user, JSON.parse(volume.config["user.permissions"])[this.user.user].includes("attach"))) {
                        return resolve(true);
                    }
                }
