...
export default class Network {
    ...
    get attach() {
        return new Promise(async (resolve, reject) => {
            ...
            if (permissions.includes("attach-network_instance")) {
                if (permissions.includes("attach_network")) {
                    return resolve(true);
                }
            }
            const nodePermissions = await this.node.permissions;
            if (nodePermissions.includes('attach-network_instance')) {
                if (nodePermissions.includes("attach_network")) {
                    return resolve(true);
                }
            }
            if (nodePermissions.includes("attach-network_instance") || permissions.includes("attach-network_instance")) {
    ...
    get edit() {
        return new Promise(async (resolve, reject) => {
            const permissions = await this.user.permissions;
            if (permissions.includes("edit_network")) {
                return resolve(true);
            }
            const nodePermissions = await this.node.permissions;
            if (nodePermissions.includes("edit_network")) {
                return resolve(true);
            }
            let network = await this.data;
            if (network) {
                if (network.metadata.config["user.permissions"]) {
                    if (JSON.parse(network.metadata.config["user.permissions"])[this.user.user].includes("edit_network")) {
                        return resolve(true);
                    }
                }
            }
