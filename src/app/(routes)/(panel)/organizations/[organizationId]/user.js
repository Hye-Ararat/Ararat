  ...
  useEffect(() => {
    ...
    if (scope == "Organization") {
      ...
      Object.keys(allPermissions).map((group) => {
        Object.keys(allPermissions[group]).map((permission) => {
          if (
            allPermissions[group][permission].scopes.includes("organization")
          ) {
            if (permissions.includes(permission)) {
              if (!newPerms[group]) {
                newPerms[group] = {};
    ...
    <>
      ...
      <SimpleGrid cols={3}>
        <TextInput
          ...
          label="Name"
          value={userName}
          disabled={!permissions.includes("update:user")}
        />
        <TextInput
      ...
      <Flex>
        ...
        <SegmentedControl
          ...
          value={scope}
          onChange={(e) => (e ? setScope(e) : setScope(scope))}
        />
      </Flex>
                ...
                <SimpleGrid cols={3}>
                  {Object.keys(activePermissions[group]).map((permission) => {
                    return (
                      <Checkbox
                        value={permission}
                        label={activePermissions[group][permission].title}
                        description={
                          activePermissions[group][permission].description
                        }
                      />
                    );
                  })}
