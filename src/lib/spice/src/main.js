...
import { handle_paste, paste, } from './clipboard.js';
...
function SpiceMainConn() {
    ...
    this.onclipboardRequest = arguments[0].onclipboardRequest
    this.onclipboardWrite = arguments[0].onclipboardWrite
...
SpiceMainConn.prototype.process_channel_message = function (msg) {
    ...
    if (msg.type == Constants.SPICE_MSG_MAIN_AGENT_DATA) {
        ...
        if (agent_data.type == Constants.VD_AGENT_ANNOUNCE_CAPABILITIES) {
            ...
            this.clipboard_request(Constants.VD_AGENT_CLIPBOARD_UTF8_TEXT)
            return true
        }
            ...
            var data = encoder.encode(this.onclipboardRequest());
            var size = data.byteLength;
            this.clipboard(data, size);
            return true
        }
...
SpiceMainConn.prototype.clipboard_request = function (type) {
    var request = new Messages.VDAgentClipboardRequest(type);
    this.send_agent_message(Constants.VD_AGENT_CLIPBOARD_REQUEST, request);
}

SpiceMainConn.prototype.clipboard = function (data, size) {

    var request = new Messages.VDAgentClipboard(Constants.VD_AGENT_CLIPBOARD_UTF8_TEXT, data, size);
...
SpiceMainConn.prototype.clipboard_release = function () {
    this.send_agent_message(Constants.VD_AGENT_CLIPBOARD_RELEASE)
}

SpiceMainConn.prototype.clipboard_grab = function () {
    var request = new Messages.VDAgentClipboardGrab([Constants.VD_AGENT_CLIPBOARD_UTF8_TEXT]);
