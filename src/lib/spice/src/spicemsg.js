...
// FIXME: need to add support for 64-bit integer for use with native Dataview
...
function VDAgentClipboardRequest(data)
{
    this.data = data;
}
...
VDAgentClipboardRequest.prototype =
{
	to_buffer: function(a, at)
	{
		at = at || 0;
		var dv = new DataView(a);
		dv.setUint32(at , this.data, true); at += 4
	},
...
function VDAgentClipboardGrab(data)
{
    this.data = data
}
...
VDAgentClipboard.prototype = 
{
    to_buffer: function(a, at)
	{
        ...
        while(i < this.size)
        {
            dv.setUint8(at, this.data[i], true); at++;
            i++;
        }
...
//some 64 int function is touch this 
