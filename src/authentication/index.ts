import Provider, { CanBePromise, Configuration } from "oidc-provider";
...
const configuration: Configuration = {
    ...
    cookies: {
        keys: [(process.env.ENC_KEY as string)],
    },
    jwks: {
    ...
    interactions: {
        url(ctx, interaction): CanBePromise<string> {
            try {
...
function errorResponse(error: number, status:number, metadata: string) {
    let response: {
        type: string,
        error: number,
        error_code: number,
...
app.post("/interaction/:uid/confirm", async (req, res, next) => {
    try {
        ...
        if (grantId) {
            ...
            if (!inter.session) return;
            grant = new oidc.Grant({
                accountId: inter.session.accountId,
                clientId: (inter.params.client_id as string)
            });
        }
        if (!grant) return;
        if (inter.prompt.details.missingOIDCScope) {
            grant.addOIDCScope((inter.prompt.details.missingOIDCScope as string[]).join(" "));
        }
        if (inter.prompt.details.missingOIDCClaims) {
            grant.addOIDCClaims((inter.prompt.details.missingOIDCClaims as string[]));
        }
